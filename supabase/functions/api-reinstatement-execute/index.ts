...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .rpc("execute_member_reinstatement", {
        p_member_id: memberId,
        p_actor_user_id: String(claims.sub || "system"),
        p_actor_role: String(claims.role || "unknown"),
      });
    ...
    if (!result) {
      return jsonResponse(500, { error: "No response from reinstatement routine" });
    }

    if (result.success !== true) {
      return jsonResponse(400, result as Record<string, unknown>);
    }

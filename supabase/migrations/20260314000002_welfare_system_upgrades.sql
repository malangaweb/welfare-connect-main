...
DECLARE
    ...
    v_row RECORD;
    v_member_id UUID;
    v_normalized_suspense_phone TEXT;
    v_normalized_member_phone TEXT;
BEGIN
    FOR v_row IN SELECT * FROM wrong_mpesa_transactions WHERE status = 'pending' LOOP
        -- Normalize suspense phone: remove +, spaces, dashes
        v_normalized_suspense_phone := REPLACE(REPLACE(REPLACE(v_row.phone_number, '+', ''), ' ', ''), '-', '');
        
        -- Try to match by normalized phone number
        SELECT id INTO v_member_id
        FROM members
        WHERE 
          -- Direct match after normalization
          REPLACE(REPLACE(REPLACE(phone_number, '+', ''), ' ', ''), '-', '') = v_normalized_suspense_phone
          OR 
          -- Handle +254 vs 07 conversion
          (phone_number LIKE '0%' AND '254' || SUBSTRING(REPLACE(REPLACE(phone_number, ' ', ''), '-', '') FROM 2) = v_normalized_suspense_phone)
          OR
          (phone_number LIKE '+254%' AND SUBSTRING(REPLACE(REPLACE(phone_number, ' ', ''), '-', '') FROM 2) = v_normalized_suspense_phone)
        LIMIT 1;

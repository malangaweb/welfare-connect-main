...
const UserSetupForm = ({ onSuccess }: UserSetupFormProps) => {
  ...
  const handleSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      // Find member by member number and get required fields
      const { data: memberRow, error: memberError } = await supabase
        .from('members')
        .select('id, member_number, name, is_active')
        .eq('member_number', values.memberNumber)
        .maybeSingle() as { data: { id: string; member_number: string; name: string; is_active: boolean } | null; error: Error | null };

...
// Extended transaction type for suspense transactions
interface SuspenseTransaction extends Transaction {
...
const SuspenseAccount = () => {
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<SuspenseTransaction | null>(null);
  ...
  const fetchSuspenseTransactions = async () => {
    ...
    try {
      ...
      const transactions: SuspenseTransaction[] = (allTransactions || []).map(tx => ({
        id: tx.id,
  ...
  const mockTransactions: SuspenseTransaction[] = [];
  const mockBalance = 0;
  const mockCredits = 0;
  const mockDebits = 0;
                ...
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge 
                        variant={tx.resolved_at ? 'default' : 'secondary'}

...
export function mapDbMemberToMember(dbMember: DbMember, dependants: any[] = []): Member {
  try {
    return {
      id: dbMember.id,
      ...
      dependants: dependants.map(dep => ({
        id: dep.id || dep.uuid || String(Math.random()),
        name: dep.name,
        gender: (dep.gender as Gender) || 'male',
        relationship: dep.relationship || 'Unknown',
        dateOfBirth: dep.date_of_birth ? new Date(dep.date_of_birth) : new Date(),
        isDisabled: dep.is_disabled || dep.isDisabled || false,
      ...
      lastLogin: dbMember.last_login ? new Date(dbMember.last_login) : undefined,
    };
  } catch (error) {
    console.error('Error in mapDbMemberToMember:', error);
    console.error('dbMember data:', dbMember);
    console.error('dependants data:', dependants);
    throw error;
  }
}

-- =====================================================
-- COMPLETE SUSPENSE ACCOUNT FIX
-- Run this in Supabase SQL Editor:
-- https://supabase.com/dashboard/project/hfojxbfcjozguobwtcgt/sql/new
-- =====================================================

-- STEP 1: Drop RLS policies (blocking frontend queries)
DROP POLICY IF EXISTS "Admins can view wrong transactions" ON wrong_mpesa_transactions;
DROP POLICY IF EXISTS "Admins can update wrong transactions" ON wrong_mpesa_transactions;
...
-- STEP 2: Fix NOT NULL constraint on phone_number
-- M-Pesa sends hashed phones, sometimes they're not available
ALTER TABLE wrong_mpesa_transactions ALTER COLUMN phone_number DROP NOT NULL;

-- STEP 3: Drop problematic check constraints
...
-- STEP 4: Drop old auto-match function (uses phone matching)
DROP FUNCTION IF EXISTS match_suspense_transactions();

-- STEP 5: Create new auto-match function (uses BillRefNumber matching)
...
BEGIN
    FOR v_row IN SELECT * FROM wrong_mpesa_transactions WHERE status = 'pending' LOOP
        ...
        IF v_member_id IS NOT NULL THEN
            ...
            -- Create transaction (wallet funding)
...
COMMENT ON FUNCTION match_suspense_transactions() IS 'Auto-matches suspense transactions by BillRefNumber (M011, 45, etc.). Skips case-only refs (C045).';

-- STEP 6: Verify the fixes
SELECT 'RLS disabled' as fix, 
       CASE WHEN rowsecurity = false THEN '✅' ELSE '❌' END as status
FROM pg_tables WHERE tablename = 'wrong_mpesa_transactions';

SELECT 'Policies dropped' as fix,
       CASE WHEN COUNT(*) = 0 THEN '✅' ELSE '❌ Still have ' || COUNT(*) END as status
FROM pg_policies WHERE tablename = 'wrong_mpesa_transactions';

SELECT 'phone_number nullable' as fix,
       CASE WHEN is_nullable = 'YES' THEN '✅' ELSE '❌' END as status
FROM information_schema.columns 
WHERE table_name = 'wrong_mpesa_transactions' AND column_name = 'phone_number';

-- STEP 7: Test - try to insert a sample record
...
-- STEP 8: Show the test record
SELECT * FROM wrong_mpesa_transactions WHERE mpesa_receipt_number = 'TEST001';

-- STEP 9: Clean up test record
DELETE FROM wrong_mpesa_transactions WHERE mpesa_receipt_number = 'TEST001';

-- STEP 10: Show current count
SELECT 'Current suspense records' as info, COUNT(*) as count
